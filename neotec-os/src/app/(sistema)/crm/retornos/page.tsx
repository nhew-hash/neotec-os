...
export default async function RetornosPage() {
  const [followups, clientes] = await Promise.all([listarFollowupsPendentes(), listarClientes()]);

  return (
    <div className="flex flex-col gap-6">
      <div>
        <h1 className="font-display text-xl font-semibold text-foreground">Agenda de retornos</h1>
        <p className="text-sm text-muted-foreground">{followups.length} retorno(s) pendente(s)</p>
      </div>

      <div className="grid grid-cols-1 gap-6 lg:grid-cols-[1fr_360px]">
        <RetornosList retornos={followups} />
