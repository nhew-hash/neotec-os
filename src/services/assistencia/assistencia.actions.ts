...
export async function criarOSAction(formData: FormData): Promise<ActionResult<{ id: string; numero: string }>> {
  ...
  try {
    ...
    // Avisa o cliente por WhatsApp que a OS foi aberta — melhor esforço,
    // nunca derruba a criação da OS se o envio falhar.
    try {
      const { data: cliente } = await supabase.from("clientes").select("nome, whatsapp").eq("id", clienteId).maybeSingle();
      if (cliente?.whatsapp) {
