...
export async function obterResumoOperacional(): Promise<ResumoOperacional> {
  ...
  const [
    ...
    supabase.from("ordens_servico").select("*", { count: "exact", head: true }).lt("prazo", hojeISO).not("status", "in", "(entregue,atendimento_encerrado)"),
    supabase.from("ordens_servico").select("*", { count: "exact", head: true }).eq("prazo", hojeISO).not("status", "in", "(entregue,atendimento_encerrado)"),
